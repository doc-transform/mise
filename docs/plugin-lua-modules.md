...
mise 插件可以使用一套全面的内置 Lua 模块，提供常见功能。这些模块在工具源插件和工具插件中都可用，使执行 HTTP 请求、JSON 解析、文件操作等常见操作变得简单。
...
semver 模块提供语义化版本比较和排序功能。这对于排序 `Available()` 钩子返回的版本列表非常有用。
...
-- 比较两个版本
-- 返回：v1 < v2 时为 -1，相等时为 0，v1 > v2 时为 1
...
-- 正确处理数值比较
local result = semver.compare("9.6.9", "9.6.24")   -- -1（不是字典序！）
...
print(full_path)  -- Unix 上：/foo/bar/baz.txt，Windows 上：\foo\bar\baz.txt
```

`file.join_path(...)` 函数使用当前操作系统的正确分隔符拼接任意数量的路径段。这是跨平台插件中构建文件路径的推荐方式。
...
> 在 Lua 中读取变量，使用 `os.getenv("MY_VAR")`。
...
- **`cwd`**（字符串）：设置命令的工作目录
- **`env`**（表）：设置命令执行的环境变量。这些会合并到继承的环境之上（见下文）。
- **`timeout`**（数字）：设置命令执行超时时间（未来功能）

### 环境模块钩子中的环境继承

当从环境模块钩子（`MiseEnv`、`MisePath`）调用 `cmd.exec()` 时，命令会自动继承 mise 构建的环境而不是进程环境。这包括前置指令设置的环境变量和到目前为止累积的 `_.path` 条目。

当模块指令设置了 `tools = true` 时，继承的环境还包括工具安装的 bin 路径。这意味着 mise 管理的工具可以直接调用：
...
function PLUGIN:MiseEnv(ctx)
    -- 启用 tools=true 后，mise 管理的工具在 PATH 上
...
不设置 `tools = true` 时，只有 `_.path` 指令条目和原始系统 PATH 可供 `cmd.exec()` 使用。

传递给 `cmd.exec()` 的显式 `env` 选项会合并到继承的环境之上，允许选择性覆盖。
...
log 模块提供结构化日志功能，通过 Rust 的 `log` crate 路由，遵循 `MISE_DEBUG` 和 `MISE_TRACE` 环境变量。
...
log.trace("detailed tracing info")   -- 仅在 MISE_TRACE=1 时可见
log.debug("debugging info")          -- 在 MISE_DEBUG=1 时可见
log.info("status message")           -- 默认可见
log.warn("warning message")          -- 默认可见
log.error("error message")           -- 默认可见
```

### 可变参数

所有日志函数接受任意类型的多个参数。参数通过 `tostring()` 转换为字符串，并用制表符（`\t`）连接，与 Lua 的 `print()` 行为一致：
...
`print()` 被重写为通过 `info!()` 级别的日志路由。这意味着：
